...
export class AdminProductsController {
  ...
  private parsePriceCents(raw: any, fallback?: number) {
    ...
    if (!Number.isFinite(numeric)) throw new BadRequestException('price must be a number');
    return Math.max(0, Math.round(numeric * 100));
  }

  private parseStock(raw: any, fallback = 0) {
    if (raw === undefined || raw === null || raw === '') return fallback;
    const numeric = Number.parseInt(String(raw), 10);
    if (!Number.isFinite(numeric)) throw new BadRequestException('stock must be a number');
    return Math.max(0, numeric);
  }

  private ensureJpeg(file?: Express.Multer.File, required = false) {
    if (!file) {
      if (required) throw new BadRequestException('Image (jpeg) is required');
      return;
    }
  ...
  private parseSpecifications(raw: any, current?: any) {
    if (raw === undefined || raw === null || raw === '') return current ?? null;
  ...
  @Get(':id')
  async getProduct(@Param('id') id: string, @Headers('authorization') authHeader?: string) {
  ...
  async updateProduct(
    ...
    if (slug !== existing.slug) {
      const existingSlug = await this.prisma.product.findUnique({ where: { slug } });
      if (existingSlug && existingSlug.id !== id) throw new BadRequestException('Slug already exists');
    }
